"""
USE FOR EXTRACTING FINGERPRINT EMBEDDINGS
...
def get_custom_loader(protocol_dir: Path, config: dict, wav_dir: Path):
    protocol_files = sorted([file for file in protocol_dir.glob("AA*.txt") if file.is_file()]) 
    loaders = {}
    for file in protocol_files:
        ...
        with open(file, 'r') as f:
            ...
            for line in lines:
                ...
                attack_type = parts[3]  # Extract attack type
                data.append((str(Path(attack_type) / "dev" / filename)))
...
if __name__ == "__main__":
    ...
    protocol_dir = Path("protocols_fingerprint_extraction")
